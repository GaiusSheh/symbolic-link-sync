...
程序首次启动时会弹出「配置 SymLiSync」向导，引导选择 `symlinks.json` 的存放位置。

**创建新配置（第一次使用）**：选择一个目录，程序在该目录下新建 `symlinks.json`。**建议选择网盘目录**（如 OneDrive、Google Drive 等），以便跨设备同步。

**使用已有配置（其他设备已使用过）**：浏览到已有的 `symlinks.json` 文件。程序读取其中的机器和链接配置，再引导注册当前机器的 Base 路径。

**跳过**：使用程序同目录下的本地 `data/symlinks.json`，跨设备同步功能不可用。

> 向导完成、注册 Base 后，若所选的 `symlinks.json` 位置不在任何已注册 Base 目录之下，程序会弹出提示。如果该目录通过网盘同步，忽略即可；否则说明配置文件无法同步到其他设备。

---

## 多机器支持与 symlinks.json 同步

**关键原则**：`symlinks.json` 存放在哪里，跨设备同步就靠哪里——程序本身不负责传输，用户自行选择网盘目录。
...
新机器首次运行时，向导引导选择已有的 `symlinks.json`（从网盘或共享位置），再注册本机 Base 路径，程序根据模板自动解析全部链接。

**各设备均需**：安装程序 + 首次运行向导 + 注册本机 Base 路径。`symlinks.json` 文件本身通过网盘自动同步，无需手动复制。
